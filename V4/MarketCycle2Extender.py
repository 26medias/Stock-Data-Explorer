...
class MarketCycleExtender:
    ...
    def addMeasurements(self, data):
        ref = self.data['Close'].shift(-1)
        cols = [
            "SIGNAL",
            "50_MAOSC",
            "100_MAOSC",
        ...
        #for val in values:
        #    cols.append(f"change-{val}")
        #data["avg-change-1"] = (data["max-gains-1d"]+data["max-loss-1d"])/2
        #data["avg-change-5"] = (data["max-gains-5d"]+data["max-loss-5d"])/2
        #data["avg-change-20"] = (data["max-gains-20d"]+data["max-loss-20d"])/2
        
        for col in cols:
            try:
                delta5 = data[col].rolling(window=5).mean()
                delta_5_diff = delta5 - delta5.shift()
                diff = data[col] - data[col].shift(1)
                data[f"m_{col}"] = (delta_5_diff * data[col].shift(1) > 0).astype(int)
            except:
                print("fail:", col)
        return data
    
    def extendGains(self, data):
        values = [x for x in range(1,21)]
        for val in values:
            data[f"change-{val}"] = (self.data['Close'].rolling(window=-val).mean()-self.data['Close'])/self.data['Close']*100
        return data
    ...
    def extend_data(self):
        ...
        self.data['avg_MAOSC'] = (self.data['50_MAOSC'] + self.data['100_MAOSC']) / 2#((self.data['Close'] - self.data['50_MA']) + (self.data['Close'] - self.data['100_MA']) + self.data['Close'] - self.data['200_MA']) / 3
        ...
        #self.data['price-1d'] = self.data['Close'].shift(-1)
        #self.data['price-5d'] = self.data['Close'].shift(-5)
        #self.data['price-20d'] = self.data['Close'].shift(-20)

        # Create 'max-gains-20d' and 'max-loss-20d' columns
        #self.data['max-gains-20d'] = self.data['Close'].rolling(window=20).apply(lambda x: ((x.max() - x[0]) / x[0]) * 100, raw=True).shift(-20)
        #self.data['max-loss-20d'] = self.data['Close'].rolling(window=20).apply(lambda x: ((x.min() - x[0]) / x[0]) * 100, raw=True).shift(-20)
        
        self.data["SIGNAL"] = (self.data['avg_MAOSC'] + self.data['DCO'] + self.data['med_MarketCycle']) / 3

        #self.data = self.extendGains(self.data)
        #self.data = self.addMeasurements(self.data)
