    ...
    def trade(self, symbols, data, onTick):
        ...
        for current_date in tqdm(unique_dates):
            ...
            if isinstance(current_data, pd.DataFrame):
                ...
                if ticker in self.positions:
                    self.positions[ticker]['gains'] = ((current_data['Close'] - self.positions[ticker]['avg_price']) / self.positions[ticker]['avg_price']) * 100
                position = self.positions.get(ticker, {'qty': 0, 'avg_price': 0, 'gains': 0})
                onTick(self, current_data, prev_row, position)
            ...
            if self.positions:
                ...
                current_data = current_data.merge(positions_df, left_on='Ticker', right_index=True, how='left')
                current_data["invested"] = current_data["qty"]*current_data["avg_price"]
                current_data["value"] = current_data["qty"]*current_data["Close"]
                current_data["gains_value"] = current_data["value"]-current_data["invested"]
                invested_value = current_data["invested"].sum(skipna=True)
                unrealized_profits = current_data["gains_value"].sum(skipna=True)
                portfolio_value = current_data["value"].sum(skipna=True)
